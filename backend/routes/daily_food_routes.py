...
from models.daily_food_log import DailyFoodEntryCreate, DailyFoodLogResponse, DailyFoodEntryResponse
...
async def add_entry(
    ...
    try:
        ...
        if not entry.product_id and not entry.recipe_id:
            raise HTTPException(status_code=400, detail="Either product_id or recipe_id must be provided")
